...
function buildContentBlocks(content = "", contentImages = []) {
  return content
    ...
    .filter(Boolean)
    .map((paragraph, index) => {
      const markerMatch = paragraph.match(/^\[\[gorsel-(\d+)(?:\|(small|medium|large))?\]\]$/i);
      ...
      if (markerMatch) {
        ...
        return {
          ...
          size: (markerMatch[2] || "medium").toLowerCase()
        };
      }
...
function renderContentBlock(block, newsTitle) {
  if (block.type === "image" && block.imageUrl) {
    return (
      <figure key={block.key} className={`detail-inline-image detail-inline-image-${block.size || "medium"}`}>
...
export default function NewsDetailPage({ news, related, previousNews, nextNews, initialComments }) {
  ...
  const hasRichContent = looksLikeHtml(news.content);
      ...
      <article className="container detail-clean-layout">
        <section className="detail-clean-main">
          ...
          <div className="detail-clean-body">
            {hasRichContent ? (
              <div className="detail-rich-body" dangerouslySetInnerHTML={{ __html: news.content }} />
            ) : (
