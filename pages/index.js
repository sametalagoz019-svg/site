...
function SectionHeading({ title }) {
  return (
    <div className="clone-section-heading">
      <span>{title}</span>
    </div>
  );
}
    ...
    <div className="clone-topbar">
      <div className="clone-topbar-inner">
        <div className="clone-brand-row">
          ...
          <span className="clone-tv-badge">Sivas odaklı güncel yayın</span>
        </div>
  ...
  return (
    <div className="clone-promo">
      <div className="clone-promo-info">
        ...
        <div>
          <strong>7/24 yayın</strong>
          <span>Manşet, sürmanşet ve öne çıkanlar</span>
        </div>
        <div>
...
function CloneHero({ items }) {
  ...
  return (
    <section className="clone-hero">
      ...
      <a href={`/news/${active.slug}`} className="clone-hero-stage" target="_blank" rel="noopener noreferrer">
        <img src={active.imageUrl || "/logo.svg"} alt={active.title} className="clone-hero-image" />
        <div className="clone-hero-overlay">
          <span className="clone-cover-badge">Kapak Görseli</span>
          <span className="clone-watch">Habere Git</span>
        </div>
      </a>
    </section>
  );
...
function CloneSponsor({ sponsor }) {
  return (
    <a href={sponsor.url || "#"} className="clone-sponsor" target="_blank" rel="noopener noreferrer">
      {sponsor.imageUrl ? <img src={sponsor.imageUrl} alt={sponsor.title} className="clone-sponsor-logo" /> : <img src="/logo.svg" alt={SITE_NAME} className="clone-sponsor-logo" />}
  ...
  return (
    <>
      ...
      <div className="clone-small-grid">
        {small.map((item) => (
          <a key={item._id} href={`/news/${item.slug}`} className="clone-small-card" target="_blank" rel="noopener noreferrer">
      ...
      <div className="clone-large-grid">
        {large.map((item) => (
          <a key={item._id} href={`/news/${item.slug}`} className="clone-large-card" target="_blank" rel="noopener noreferrer">
            <img src={item.imageUrl || "/logo.svg"} alt={item.title} />
            <strong>{item.title}</strong>
          </a>
        ))}
...
function CloneSecondHero({ items }) {
  ...
  return (
    <section className="clone-second">
      <SectionHeading title="Sürmanşet" />
      ...
      <a href={`/news/${active.slug}`} className="clone-second-stage" target="_blank" rel="noopener noreferrer">
        <img src={active.imageUrl || "/logo.svg"} alt={active.title} className="clone-second-image" />
        <div className="clone-second-overlay">
          <span className="clone-cover-badge clone-cover-badge-bottom">Kapak Görseli</span>
          <span className="clone-watch clone-watch-bottom">Detayı Oku</span>
        </div>
      </a>
    ...
    <section className="clone-compact-news">
      <div className="clone-compact-grid">
        {compactItems.map((item) => (
          <a key={item._id} href={`/news/${item.slug}`} className="clone-compact-card" target="_blank" rel="noopener noreferrer">
            <img src={item.imageUrl || "/logo.svg"} alt={item.title} />
            <div>
    ...
    <section className="clone-columns">
      <div className="clone-col">
        {columns.slice(0, 3).map((item) => (
          <a key={item._id} href={`/news/${item.slug}`} className="clone-col-card" target="_blank" rel="noopener noreferrer">
            <img src={item.imageUrl || "/logo.svg"} alt={item.title} />
            <strong>{item.title}</strong>
          </a>
        ))}
      ...
      <aside className="clone-mostread">
        ...
        {mostRead.slice(0, 5).map((item) => (
          <a key={item._id} href={`/news/${item.slug}`} className="clone-mostread-item" target="_blank" rel="noopener noreferrer">
            <img src={item.imageUrl || "/logo.svg"} alt={item.title} />
            <strong>{item.title}</strong>
          </a>
        ))}
