...
export default function AdminNewsPage({ publishedNews, pendingNews, draftNews }) {
  ...
  async function generateSampleNews() {
    ...
    const response = await fetch("/api/admin/generate-sample", { method: "POST" });
    const payload = await response.json();
    setLoading(false);
    setMessage(payload.message || "Örnek haber oluşturuldu.");
    ...
    <AdminLayout title="Haber Merkezi">
      <section className="panel admin-intro admin-intro-pro">
        <div>
          ...
          <h2>Haberleri kart görünümüyle düzenle, sırala ve yayına al</h2>
          <p>Görsel, başlık, kategori, tarih ve hızlı aksiyonlar tek kart içinde. Sürükle-bırak ile yayın sırasını yönetebilirsin.</p>
        </div>
      ...
      <div className="action-bar">
        ...
        <button type="button" className="button button-outline" onClick={generateSampleNews} disabled={loading}>
          Örnek Veri Üret
        </button>
      </div>
