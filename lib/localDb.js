...
const BUNDLED_DATA_FILE = path.join(BUNDLED_DATA_DIR, "localdb.json");

function getRuntimeRoot() {
  if (process.env.VERCEL) {
    return path.join(process.env.TMPDIR || "/tmp", "sivas-gundem-58");
  }
...
function getDataFile() {
  return path.join(getDataDir(), "localdb.json");
}
